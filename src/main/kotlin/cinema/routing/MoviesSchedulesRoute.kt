...
fun Route.movieSchedulesRoute(
    ...
    route("/movie_schedules") {
        ...
        get("{movie_id}") {
            val movieId = call.parameters["movie_id"] ?: return@get call.respond(
                HttpStatusCode.BadRequest,
                "Movie ID is required"
            )
            val movie = movieScheduleService.getMoviesWithSchedules(movieId) ?: return@get call.respond(
        ...
        authenticate {
            requireRoles(UserRole.ADMIN) {
                ...
                delete("{movie_id}") {
                    val movieId = call.parameters["movie_id"] ?: return@delete call.respond(
                        HttpStatusCode.BadRequest,
                        "Movie ID is required"
                    )
    ...
    authenticate {
        requireRoles(UserRole.ADMIN) {
            route("/schedule/{schedule_id}") {
                delete {
                    val scheduleId = call.parameters["schedule_id"]?.toLong() ?: return@delete call.respond(
                        HttpStatusCode.BadRequest,
                        "Movie ID is required"
                    )
